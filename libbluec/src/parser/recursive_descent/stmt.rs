    ...
    match parser.token_stream.peek_next_2_tokens() {
        ...
        (Some(token), _) => {
            match token.token_type {
                ...
                // Control statement
                lexer::TokenType::Identifier(ref id) if utils::is_control_statement_keyword(id) => match id.as_str() {
...
pub fn parse_if_statement(parser: &mut Parser, driver: &mut Driver) -> ParseResult<AstStatement> {
    let if_loc = utils::expect_identifier_token_with_name("if", parser, driver)?;
    ...
    //                 return 0;
    //
    let else_stmt = if parser.token_stream.next_token_is_identifier("else") {
    ...
    let node_id = driver.make_node_id();
    parser.metadata.add_source_location(node_id, if_loc);

    Ok(AstStatement::If { node_id, controlling_expr, then_stmt, else_stmt })
}
...
pub fn parse_switch_statement(parser: &mut Parser, driver: &mut Driver) -> ParseResult<AstStatement> {
    let switch_loc = utils::expect_identifier_token_with_name("switch", parser, driver)?;
    _ = utils::expect_token(lexer::TokenType::OpenParen, parser, driver)?;
    let controlling_expr = expr::parse_expression(parser, driver)?;
    _ = utils::expect_token(lexer::TokenType::CloseParen, parser, driver)?;

    let node_id = driver.make_node_id();
    parser.metadata.add_source_location(node_id, switch_loc);
...
pub fn parse_switch_case_statement(parser: &mut Parser, driver: &mut Driver) -> ParseResult<AstStatement> {
    let case_token_loc = utils::expect_identifier_token_with_name("case", parser, driver)?;
...
pub fn parse_switch_default_statement(parser: &mut Parser, driver: &mut Driver) -> ParseResult<AstStatement> {
    let default_token_loc = utils::expect_identifier_token_with_name("default", parser, driver)?;
...
pub fn parse_while_statement(parser: &mut Parser, driver: &mut Driver) -> ParseResult<AstStatement> {
    let while_loc = utils::expect_identifier_token_with_name("while", parser, driver)?;
    ...
    let node_id = driver.make_node_id();
    parser.metadata.add_source_location(node_id, while_loc);
...
pub fn parse_do_while_statement(parser: &mut Parser, driver: &mut Driver) -> ParseResult<AstStatement> {
    let do_loc = utils::expect_identifier_token_with_name("do", parser, driver)?;

    let node_id = driver.make_node_id();
    parser.metadata.add_source_location(node_id, do_loc);

    // Loop body
    let body = parser.with_enclosing_statement(EnclosingStatement::Loop(node_id), |p| parse_statement(p, driver))?;

    _ = utils::expect_identifier_token_with_name("while", parser, driver)?;
    let open_paren_loc = utils::expect_token(lexer::TokenType::OpenParen, parser, driver)?;
...
pub fn parse_for_statement(parser: &mut Parser, driver: &mut Driver) -> ParseResult<AstStatement> {
    parser.with_new_scope(|parser| {
        let for_loc = utils::expect_identifier_token_with_name("for", parser, driver)?;
        _ = utils::expect_token(lexer::TokenType::OpenParen, parser, driver)?;
        ...
        let node_id = driver.make_node_id();
        parser.metadata.add_source_location(node_id, for_loc);
...
pub fn parse_break_statement(parser: &mut Parser, driver: &mut Driver) -> ParseResult<AstStatement> {
    let break_token_loc = utils::expect_identifier_token_with_name("break", parser, driver)?;
...
pub fn parse_continue_statement(parser: &mut Parser, driver: &mut Driver) -> ParseResult<AstStatement> {
    let continue_token_loc = utils::expect_identifier_token_with_name("continue", parser, driver)?;
...
pub fn parse_goto_statement(parser: &mut Parser, driver: &mut Driver) -> ParseResult<AstStatement> {
    let goto_token_loc = utils::expect_identifier_token_with_name("goto", parser, driver)?;
    let label_token = utils::expect_identifier(parser, driver)?;
...
pub fn parse_return_statement(parser: &mut Parser, driver: &mut Driver) -> ParseResult<AstStatement> {
    let return_loc = utils::expect_identifier_token_with_name("return", parser, driver)?;

    let expr = if parser.token_stream.next_token_has_type(lexer::TokenType::Semicolon) {
        None
    } else {
        let expr = expr::parse_expression(parser, driver)?;
        Some(expr)
    ...
    let node_id = driver.make_node_id();
    parser.metadata.add_source_location(node_id, return_loc);

    Ok(AstStatement::Return { node_id, expr })
}
