/* Test that we correctly find the common type in expressions involving long doubles */
...
int lt(long double d, long l) {
    // l is implicitly converted to a double
    return d < l;
}

long double tern_double_flag(long double flag) {
     ...
     */
    return (long double) (flag ? -30 : 10ul);
}

long double tern_double_result(int flag) {
    /* The common type of the two operands is double,
     * so if flag is 0, this will return the nearest representable
     * long double to 9223372036854777850ul, which is 9223372036854777856.0
     */
    return flag ? 5.0 : 9223372036854777850ul;
}
int ten = 10;
int multiply(void) {
    /* This should promote 10 to a long double,
...
int main(void) {
     ...
     */
    if (lt(-9007199254751228.0L, -9007199254751227l)) {
        return 1;
    }

    /* Ternary expressions */
    if (tern_double_flag(20.0L) != 18446744073709551586.0L) {
        return 2;
    }

    if (tern_double_flag(0.0L) != 10.0L) {
        return 3;
    }

    if (tern_double_result(1) != 5.0L) {
        return 4;
    }
    if (tern_double_result(0) != 9223372036854777856.0L) {
        return 5;
    }
