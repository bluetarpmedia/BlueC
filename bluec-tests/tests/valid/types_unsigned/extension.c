...
int ushort_to_long(unsigned short us, long expected) {
    long result = (long) us;
    return result == expected;
}

int ushort_to_ulong(unsigned short us, unsigned long expected){
    return (unsigned long) us == expected;
}
...
int main(void) {
    ...
    /* Extending an unsigned short to a signed long preserves its value */
    if (!ushort_to_long(65535, 65535l)) {
        return 6;
    }

    /* Extending an unsigned short to an unsigned long preserves its value */
    if (!ushort_to_ulong(65535, 65535ul)) {
        return 7;
    }
